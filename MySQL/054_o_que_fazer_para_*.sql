...
SELECT 
   ...
   GROUP_CONCAT(name ORDER BY name SEPARATOR ', ') as nomes_tours
FROM tour
GROUP BY stop
HAVING COUNT(*) > 1
...
SELECT 
   id,
   CONCAT(name, ' (', type, ')') as owner_info
FROM owner
LIMIT 10;
...
SELECT 
   w.id,
   CONCAT(w.name, ' - ', c.name, ' County') as localizacao_completa
FROM waterfall w
INNER JOIN county c ON w.county_id = c.id
LIMIT 10;
...
SELECT 
   ...
   CONCAT(
      name,
      COALESCE(CONCAT(' | Phone: ', phone), ' | Phone: N/A'),
      ' | Type: ',
      type
   ) as owner_completo
FROM owner
...
-- Usando CONCAT_WS (concat with separator) - ignora NULLs automaticamente
SELECT 
   id,
   CONCAT_WS(' | ', name, phone, type) as owner_info
FROM owner
LIMIT 10;

-- 3.2. Concatenar campos de várias linhas em um único resultado
-- Listar todos os nomes de cachoeiras em uma única string
SELECT GROUP_CONCAT(name ORDER BY name SEPARATOR ', ') as todas_cachoeiras
FROM waterfall;

-- Limitar o tamanho do resultado (primeiras 10)
SELECT GROUP_CONCAT(name ORDER BY name SEPARATOR ', ') as primeiras_cachoeiras
...
SELECT 
   ...
   GROUP_CONCAT(w.name ORDER BY w.name SEPARATOR ' | ') as lista_cachoeiras
FROM county c
...
SELECT 
   ...
   GROUP_CONCAT(t.name ORDER BY t.name SEPARATOR '; ') as tours_disponiveis
FROM waterfall w
...
CREATE TEMPORARY TABLE waterfall_backup AS
...
INNER JOIN county c ON wb.county_id = c.id
SET wb.county_population = c.population;
...
DROP TEMPORARY TABLE IF EXISTS waterfall_backup;
CREATE TEMPORARY TABLE waterfall_backup AS
SELECT * FROM waterfall;
...
INNER JOIN county c ON w.county_id = c.id
SET w.owner_id = 2  -- Governo Municipal
...
CREATE TEMPORARY TABLE county_stats AS
SELECT 
   county_id,
...
DROP TEMPORARY TABLE IF EXISTS waterfall_backup;
DROP TEMPORARY TABLE IF EXISTS county_stats;
...
GROUP BY t1.id
HAVING id_faltando_inicio < MIN(t2.id)
ORDER BY id_faltando_inicio;
