...
SELECT 
   name,
   population,
   ROUND(population / 1000) AS milhares_arredondado
FROM county
LIMIT 5;
...
SELECT 
   name,
   population,
   ROUND(population / 1000, 2) AS milhares_2_decimais,
   ROUND(population / 1000000, 3) AS milhoes_3_decimais
FROM county
LIMIT 5;
...
SELECT 
   name,
   population / 1000 AS valor_exato,
   CEIL(population / 1000) AS arredondado_cima
FROM county
LIMIT 5;
...
SELECT 
   name,
   population / 1000 AS valor_exato,
   FLOOR(population / 1000) AS arredondado_baixo
FROM county
LIMIT 5;
...
-- TRUNCATE() - Remove casas decimais sem arredondar

-- Truncar população
SELECT 
   name,
   population / 1000 AS valor_exato,
   TRUNCATE(population / 1000, 0) AS truncado_inteiro,
   TRUNCATE(population / 1000, 1) AS truncado_1_decimal
FROM county
LIMIT 5;
...
SELECT 
   ...
   ROUND(population / 1000, 1) - TRUNCATE(population / 1000, 1) AS diferenca
FROM county
LIMIT 8;
...
SELECT 
   ...
   TRUNCATE(4.7, 0) AS truncate_result
UNION ALL
SELECT 
   'Negativo -4.7',
   ROUND(-4.7),
...
SELECT 
   name,
   population,
   FLOOR(population / 100000) * 100000 AS faixa_inferior,
   CEIL(population / 100000) * 100000 AS faixa_superior
FROM county
LIMIT 10;
...
SELECT 
   id,
   name,
   ROUND(id / 5) * 5 AS id_arredondado_5
FROM waterfall
LIMIT 10;
...
SELECT 
   type,
   COUNT(*) AS total,
   ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM owner), 0) AS porcentagem_0_dec,
   ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM owner), 1) AS porcentagem_1_dec,
   ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM owner), 2) AS porcentagem_2_dec
FROM owner
GROUP BY type;
...
SELECT 
   ...
   population,
   ROUND(
      (population - (SELECT MIN(population) FROM county)) * 10.0 / 
