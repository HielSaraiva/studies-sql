...
WITH RECURSIVE hierarquia_populacional AS (
    ...
    SELECT 
        ...
        CAST(name AS CHAR(200)) AS caminho
    FROM county
    ...
    SELECT 
        ...
        hp.nivel + 1 AS nivel,
        'Grande' AS categoria,
        CONCAT(hp.caminho, ' > ', c.name) AS caminho
    FROM county c
    INNER JOIN hierarquia_populacional hp 
        ON c.population BETWEEN hp.population * 0.3 AND hp.population * 0.7
...
CREATE TEMPORARY TABLE temp_dados_preparados AS
SELECT o.id, o.name, o.type, COUNT(w.id) AS total
FROM owner o
LEFT JOIN waterfall w ON o.id = w.owner_id
GROUP BY o.id, o.name, o.type;
...
DROP TEMPORARY TABLE IF EXISTS temp_dados_preparados;
...
-- 2. CTEs recursivas têm limite de profundidade (cte_max_recursion_depth)
...
-- Exemplo de CTE com limite de recursão aumentado
