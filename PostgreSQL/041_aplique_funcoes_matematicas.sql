...
SELECT 
   name,
   population,
   ROUND(CAST(SQRT(population) AS numeric), 2) AS raiz_quadrada,
   ROUND(CAST(population / SQRT(population) AS numeric), 2) AS densidade_relativa
FROM county
ORDER BY population DESC
LIMIT 10;
...
SELECT 
   ...
   name,
   population,
   ROUND(CAST(EXP(1) AS numeric), 4) AS numero_euler,
   ROUND(CAST(population * EXP(0.05) AS numeric), 2) AS projecao_crescimento_5pct
FROM county
LIMIT 10;

-- LOG() - Logaritmo (no PostgreSQL, LOG é base 10 por padrão)

-- Logaritmo base 10 da população
SELECT 
   name,
   population,
   ROUND(CAST(LOG(population) AS numeric), 4) AS log_base10,
   ROUND(CAST(LN(population) / LN(10) AS numeric), 4) AS log_base10_calculado
FROM county
ORDER BY population DESC
LIMIT 10;
...
-- LN é o logaritmo natural no PostgreSQL
SELECT 
   name,
   population,
   ROUND(CAST(LN(population) AS numeric), 4) AS ln_populacao,
   ROUND(CAST(LOG(population) AS numeric), 4) AS log10_populacao
FROM county
LIMIT 10;

-- LOG() - Logaritmo base 10 (no PostgreSQL, LOG é base 10)

-- Escala logarítmica da população
SELECT 
   name,
   population,
   ROUND(CAST(LOG(population) AS numeric), 2) AS escala_log10,
   FLOOR(LOG(population)) AS ordem_magnitude
FROM county
ORDER BY population DESC
LIMIT 15;
...
SELECT 
   name,
   population,
   ROUND(CAST(PI() AS numeric), 6) AS pi,
   ROUND(CAST(PI() * POWER(SQRT(population/1000), 2) AS numeric), 2) AS area_proporcional
FROM county
LIMIT 10;
...
SELECT 
   name,
   population,
   ROUND(CAST(population AS numeric) / 1000, 0) AS milhares,
   ROUND(CAST(population AS numeric) / 1000, 1) AS milhares_1_decimal,
   ROUND(CAST(population AS numeric) / 1000000, 2) AS milhoes_2_decimais
FROM county
LIMIT 10;

-- TRUNC() - Truncar casas decimais
...
SELECT 
   30,
   RADIANS(30),
   ROUND(CAST(SIN(RADIANS(30)) AS numeric), 4),
   ROUND(CAST(COS(RADIANS(30)) AS numeric), 4),
   ROUND(CAST(TAN(RADIANS(30)) AS numeric), 4)
UNION ALL
SELECT 
   45,
   RADIANS(45),
   ROUND(CAST(SIN(RADIANS(45)) AS numeric), 4),
   ROUND(CAST(COS(RADIANS(45)) AS numeric), 4),
   ROUND(CAST(TAN(RADIANS(45)) AS numeric), 4)
UNION ALL
SELECT 
   60,
   RADIANS(60),
   ROUND(CAST(SIN(RADIANS(60)) AS numeric), 4),
   ROUND(CAST(COS(RADIANS(60)) AS numeric), 4),
   ROUND(CAST(TAN(RADIANS(60)) AS numeric), 4)
UNION ALL
SELECT 
   90,
   RADIANS(90),
   ROUND(CAST(SIN(RADIANS(90)) AS numeric), 4),
   ROUND(CAST(COS(RADIANS(90)) AS numeric), 4),
   NULL;
...
SELECT 
   ...
   ROUND(CAST(AVG(population) AS numeric), 2) AS media,
   ROUND(CAST(SQRT(AVG(POWER(population - (SELECT AVG(population) FROM county), 2))) AS numeric), 2) AS desvio_padrao,
...
SELECT 
   ...
   population,
   ROUND(
      CAST((population - (SELECT MIN(population) FROM county)) AS numeric) / 
