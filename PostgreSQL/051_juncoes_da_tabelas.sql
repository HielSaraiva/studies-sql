...
-- Não requer cláusula ON - PostgreSQL encontra automaticamente as colunas comuns
...
CREATE TEMP TABLE temp_waterfall_stats (
    id INT,
    name VARCHAR(30),
    visitor_count INT
);

CREATE TEMP TABLE temp_waterfall_reviews (
    id INT,
    name VARCHAR(30),
    rating DECIMAL(3,2)
);
...
DROP TABLE IF EXISTS temp_waterfall_stats, temp_waterfall_reviews;
...
CREATE TEMP TABLE waterfall_normalized (
    waterfall_id INT,
    name VARCHAR(30)
);

CREATE TEMP TABLE waterfall_location (
    waterfall_id INT,
...
CREATE TEMP TABLE waterfall_details (
    waterfall_id INT,
    name VARCHAR(30),
    height_meters INT
);
...
-- Limpar tabelas temporárias
DROP TABLE IF EXISTS waterfall_normalized, 
                     waterfall_location, 
                     waterfall_details;
